...
function update_scoreboard() {
  ...
  for (let i = 1; i < scoreboard.length; i++) {
    if (scoreboard[i]) {
      ...
      const balls = scoreboard[i].slice(1).map(b => b === undefined ? "" : b).join(" - ");
      const extras = scoreboard[i][0] || 0;
      const scoreCell = $("<td></td>").text(`<span class="math-inline">\{balls\} \(</span>{extras})`);
...
function onMessageArrived(message) {
  try {
    ...
    if (payload.type === "init") {
      ...
      targetOvers = state.targetOvers;
      updateHtml("#over-ball", state.overBallDisplay, false);
      updateHtml("#run", state.runDisplay, false);
      updateHtml("#wickets", state.wicketsDisplay, false);
      for (const key in state.ballButtons) {
        updateHtml(key, state.ballButtons[key], false);
        $(key).removeClass("btn-primary").addClass("btn-light");
      }
      if (ball_no >= 1 && ball_no <= 6) {
        $("#ball_no_" + ball_no).removeClass("btn-light").addClass("btn-primary");
      }
      if (state.targetBoardVisible) $("#targetBoard").show(); else $("#targetBoard").hide();
